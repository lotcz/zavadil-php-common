...
class OAuthTokenManager
{
    ...
    private function getExistingAccessToken(string $scope): ?AccessTokenPayload
    {
        // todo: add scope/permission check
        if (!isset($this->accessTokens[$privilege])) {
            return null;
        }
    ...
    {
        $this->reset();
        $payload = new RequestRefreshTokenFromLoginPayload(
            $this->audience,
            $this->login,
            $this->password,
        );
        $this->refreshToken = $this->oAuthServer->requestRefreshTokenFromLogin(
            $payload,
        );
    ...
    public function getRefreshToken(): RefreshTokenPayload
    {
        ...
        if (OAuthHelper::isTokenReadyForRefresh($this->refreshToken)) {
    ...
    public function getAccessToken(string $privilege): AccessTokenPayload
    {
        ...
        if (
            OAuthHelper::isValidToken($existing) &&
            !OAuthHelper::isTokenReadyForRefresh($existing)
        ) {
            return $existing;
        }
